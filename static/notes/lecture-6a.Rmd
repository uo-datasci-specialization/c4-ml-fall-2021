...
- Obsevation 1: $\mathbf{B} = (B_1, B_2, B_3, ..., B_P)$
...
for(i in 1:2834){
  ...
  b <- as.matrix(baked_read[dist[i,1],1:887])
...
for(i in 1:18110){
  ...
  b <- as.matrix(baked_recidivism[dist2[i,1],2:165])
...
# 2. K Nearest Neighbors for Continuous Outcomes
...
# Hyperparameter Tuning Grid
  ...
  # check the hyperparameters for the kknn method
  # getModelInfo()$kknn$parameters
  # getModelInfo()$kknn$grid
...
# Train the model
  ...
  caret_knn <- caret::train(blueprint_readability, 
                            data      = read_tr,
                            method    = "kknn",
                            trControl = cv,
                            tuneGrid  = grid)
...
# 3. K Nearest Neighbors for Binary Outcomes
