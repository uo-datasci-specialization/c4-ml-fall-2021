...
The 10-fold cross-validation results on the training dataset indicate that a $\lambda$ value of 0.63 provides the best performance (minimum RMSE). Let's use this model to predict the outcome in the hold-out test dataset.
...
It looks like a $\lambda$ value of 0.006 is the best pick although any number between 0.005 and 0.010 probably works fine. Let's now use the lasso regression model to predict the outcome in the hold-out test dataset as we did before for the ridge regression.
